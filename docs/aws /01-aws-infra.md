...
flowchart TB
    ...
    subgraph REGION["AWS region &middot; us-east-1"]
        ...
        R53["Route53 hosted zone<br/>vijaygiduthuri.in<br/>delegated in Phase 5"]
    end
...
| `domain_name` | `vijaygiduthuri.in` | Apex domain, delegated from GoDaddy in Phase 5 |
| `subdomain` | `""` (empty) | Empty serves the **zone apex**, `vijaygiduthuri.in`. A name like `eventhub` would give `eventhub.vijaygiduthuri.in`. Only one environment can own the apex. |
